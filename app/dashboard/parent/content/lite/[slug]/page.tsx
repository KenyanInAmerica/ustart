...
export default async function ParentLiteModulePage({
  params,
  searchParams,
}: {
  params: { slug: string };
    ...
    <div>
      ...
      <div className="flex items-center justify-between mt-12 pt-6 border-t border-[var(--border)]">
        <div>
          {prevModule && (
            <Link
              href={`/dashboard/parent/content/lite/${prevModule.slug}`}
        ...
        <div>
          {nextModule && (
            <Link
              href={`/dashboard/parent/content/lite/${nextModule.slug}`}
